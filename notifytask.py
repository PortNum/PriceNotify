...
def get_current_cfd_price(code: str):
    ...
    try:
        ...
        print("获取 %s 当前价格失败" % code)
        print(e)
        return None


def get_current_cn_stock_price(symbol: str):
    try:
        stock_prices = ak.stock_bid_ask_em(symbol=symbol)
...
def is_cn_stock_opening_time():
    now_localtime = time.strftime("%H:%M:%S", time.localtime())
    if "09:30:00" < now_localtime < "11:30:00" or "13:00:00" < now_localtime < "15:00:00":
        return True
    print("当前时间A股未开盘: %s" % now_localtime)
    return False


def compare_and_send(current_price, target_price, compare_direction, symbol, name):
    try:
        if compare_price(current_price=current_price, target_price=target_price,
